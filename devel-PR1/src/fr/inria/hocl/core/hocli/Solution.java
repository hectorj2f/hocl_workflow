...
public class Solution implements Atom, Serializable {
	...
	public Solution() {
		...
		strategy = Hocli.newStrategy();
	}


	public SimpleLinkedList<SolutionObserver> getObserversAdd() {
		return observersAdd;
	}
	...
	/**
	 * 
	 * @return a new Solution object with all the elements clon
	 */
	public Solution clone()  {
		Solution sol = new Solution();
		
		sol.addMolecule(contents.clone());
						
		for(int it=0; it < allTypes.size(); it++){
			String type = new String(allTypes.get(it));
			sol.addToAllTypes(type);
		}
		
		sol.initInnerSolutions();
		
		sol.setStrategy(Hocli.newStrategy());
	...
	public void reduce() {
		initInnerSolutions();
		if( isInnerSolutions() ) {
			for( int it = 0; it < innerSolutions.size(); it++ ) {
				Solution innerSolution = innerSolutions.get( it );
