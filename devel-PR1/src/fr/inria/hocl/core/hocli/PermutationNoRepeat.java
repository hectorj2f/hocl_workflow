...
public class PermutationNoRepeat implements Permutation, Serializable {
	...
	/**
	 * @param size
	 *          Size of the permutation (number of atoms looked for)
	 * @param s
	 *          Solution to enumerate
	 */
	public PermutationNoRepeat( AtomIterator[] atomIterators,
			MoleculeIterator[] moleculeIterators, ReactionRule reactionRule ) {
	...
	public Molecule getReactives() {
		...
		for( int i = 0; i < moleculeIterators.length; i++ ) {
			molecule.add( moleculeIterators[i].getMolecule() );
		}
		return molecule;
	}
	...
	private boolean moveIterIfConflict( int itNb ) {
		...
		if( isIteratorAtCycleEnd( itNb ) ) {
			...
			if( j < itNb ) {
				// found a conflict, find next permutation
				exists = nextHard( itNb );
			} else {
				exists = true;
			}
		}
	...
	private void computeMolecules() {
		...
		if( moleculeIterators.length > 0 ) {
			...
			for( int i = 0; i < moleculeIterators.length; i++ ) {
				...
				if( moleculeIterators[i].getFraction() == 0 ) {
					size = 0;
				} else {
					size = solution.size() / moleculeIterators[i].getFraction();
				}

				moleculeIterators[i].clear();
				...
				for( int j = 0; j < size && !it_sol.isAtEnd(); j++ ) {
					...
					if( found ) {
						moleculeIterators[i].addAtom( it_sol.getElement() );
						nbRemainingAtoms--;
						it_sol.next();
					}
				}
