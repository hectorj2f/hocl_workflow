...
public class VarDescReactionRule implements VarDesc {
	...
	public boolean equals( VarDesc varDesc ) {
		...
		if( varDesc instanceof VarDescReactionRule ) {
			equals = ( ( ( VarDescReactionRule ) varDesc ).ident == ident );
		}
		return equals;
	}
	...
	public String generateNew() {
		...
		Iterator<String> it_arg = arguments.iterator();
		while( it_arg.hasNext() ) {
			args = args + ", " + it_arg.next();
		}
		if( args.length() > 0 ) {
			args = args.substring( 2 );
		}
		s = "new " + Hoclc2j.upperFirstLetter( reactionName ) + "(" + args + ")";
		return s;
	}
	...
	public String generatePatClass( Pattern allDesc, Set<String> vars,
			String permutation ) {
		return new String(); // cf fr.inria.hocl.core.hocli.PatName
	}


	public String generateNewPat() {
		return "new IteratorForRule(\"" + reactionName + "\","
				+ Hoclc2j.upperFirstLetter( ruleName ) + ".this)";
	}
	...
	public String generateClassDecl( Pattern patDesc,
		...
		s = "class " + Hoclc2j.upperFirstLetter( reactionName )
				+ " extends ReactionRule implements Serializable {\n\n"
				+ Hoclc2j.indentCode( attributs ) + "\n"
				+ Hoclc2j.indentCode( generateConstructor( patDesc, sizeResult ) )
				+ "\n";
		...
		s = s + "\tprotected Molecule computeResult(){\n"
				...
				+ "\t\treturn " + resMol + ";\n" + "\t}\n";
		s = s + "\n" + "} // class " + Hoclc2j.upperFirstLetter( reactionName );
		return s;
	}
	...
	private String generateConstructor( Pattern patDesc, int sizeResult ) {
		 ...
		 * "OPTIMIZER"; }
		 */
		s = "public " + Hoclc2j.upperFirstLetter( reactionName ) + "(" + args
				+ "){\n" + "\tsuper(\"" + reactionName + "\", Shot." + shot + ");\n"
				+ "\tsetTrope(Trope." + trope + ");\n" + Hoclc2j.indentCode( initArgs )
				+
