		...
		while( it.hasNext() ) {
			...
			if( atom.getType().equals( "tuple" ) ) {
				for( int i = 0; i < ( ( Tuple ) atom ).contents.size(); i++ ) {
					...
					t = ( ( Tuple ) atom ).contents.get( i ).getElementTypes();
					if( t != null ) {
						for( int j = 0; j < t.size(); j++ ) {
							addToLinkedList( types, t.get( j ) );
						}
					}
				}
				...
				if( ( ( External ) atom ).toString().length() > 3
						&& ( ( External ) atom ).toString().substring( 0, 3 )
								.equals( "new" ) ) {
					String[] s = ( ( External ) atom ).toString().split( "\\(" );
					String[] t = s[0].split( "\\ " );
					type = t[1];
				}
			...
			//	if( !type.equals( "IOSender" ) )
				...
				LinkedList<String> s = ( ( BasicSolution ) atom ).getElementTypes();
				for( int i = 0; i < s.size(); i++ ) {
					addToLinkedList( types, s.get( i ) );
				}
...
*/
	...
	public String generateDeclarationInit() {
		...
		while( it.hasNext() ) {
			...
			// Cloneable argument for the compiler, the system creates clones for each atom.
			//TODO: Check the performance consumption using clones for each atom.
			String generateClone="";			
			
			if ( Hoclc2j.cloneable ){
				generateClone = ".clone()";
			}
			...
			if( atom.getType().equals( "tuple" ) ) {
				code += "tuple = " + atom.generateReference() + ";\n" + varName
						+ ".add(" + "tuple" + generateClone+");\n" + "this.addType(\"Tuple\");\n\n";
			} else if( atom.getType().equals( "external" ) ) {
				
				//FIXME: Hector Tuple, Solution are supported type as return value in a javaExpresion
				code += "if ("+atom.generateReference()+" instanceof ExternalObject) {\n";
				code += "\tExternalObject auxObject = (ExternalObject) "	
				+ atom.generateReference()
					+ ";\n"
					+ "\t"+varName
					+ ".add("
					+ "auxObject"+generateClone
					+ ");\n"
					+ "\tstring = auxObject.getObject().getClass().toString().split(\"\\\\.\");\n"
					+ "\tthis.addType(string[string.length-1]);\n} else {\n";
				
				code+= "\t"+varName+".add("+atom.generateReference()+generateClone+ ");\n"
					+"if("+atom.generateReference()+" instanceof Tuple) \n\tthis.addType(\"Tuple\");\n"
					+"}\n";	
